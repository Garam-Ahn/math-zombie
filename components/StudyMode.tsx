...
export const StudyMode: React.FC<StudyModeProps> = ({ onBack, onPlay, isIngame = false }) => {
  ...
  return (
    <div className="h-[100dvh] w-screen bg-[#1a4731] relative flex flex-col items-center justify-center p-2 md:p-4 font-sans overflow-hidden z-[70]">
      ...
      <div className="absolute top-10 left-10 w-32 h-32 opacity-20 pointer-events-none animate-float" dangerouslySetInnerHTML={{ __html: SVG_SUNFLOWER(1) }}></div>
      ...
      <div className="z-10 w-full max-w-6xl flex justify-between items-center mb-2 md:mb-4 lg:mb-6 flex-shrink-0">
        <button onClick={onBack} className="text-white text-lg md:text-xl font-black hover:text-green-300 drop-shadow-md uppercase tracking-widest">
            {isIngame ? "← RESUME" : "← BACK"}
        </button>
        ...
        <button onClick={onPlay} className="glossy-btn bg-yellow-500 text-yellow-900 px-4 py-2 md:px-8 md:py-3 text-sm md:text-xl font-black animate-pulse whitespace-nowrap">
            {isIngame ? "CONTINUE ▶" : "PLAY ▶"}
        </button>
      </div>
      ...
      <div className="flex flex-row gap-4 w-full max-w-6xl flex-1 min-h-0 items-stretch">
          ...
          <div className="flex flex-col justify-center gap-2 p-3 md:p-4 wood-panel bg-[#3e2723] flex-shrink-0 overflow-y-auto w-20 md:w-auto scrollbar-hide">
              <h3 className="text-center text-yellow-200 font-black mb-1 md:mb-2 text-xs md:text-base tracking-tighter">TABLE</h3>
          ...
          <div className="flex-1 wood-panel bg-[#fefce8] relative overflow-hidden flex flex-col md:flex-row p-4 md:p-6 lg:p-8 items-center justify-center shadow-2xl border-4 border-[#854d0e]">
                ...
                <div className="flex-1 flex flex-col items-center justify-center z-10 w-full h-full min-h-0">
                    <div className="text-xl md:text-2xl lg:text-3xl font-black text-green-700 mb-2 md:mb-4 uppercase tracking-widest">
                        Focus Pattern
                    </div>
                            ...
                            {Array.from({ length: activeIndex + 1 }).map((_, groupIdx) => (
                                <div key={groupIdx} className="flex flex-col gap-0.5 md:gap-1 bg-blue-100 p-0.5 md:p-1 rounded border border-blue-200 animate-[fadeIn_0.5s_ease-out]" style={{ animationDelay: `${groupIdx * 0.1}s` }}>
                                    {Array.from({ length: selectedTable }).map((_, dotIdx) => (
                                        <div key={dotIdx} className="w-2.5 h-2.5 md:w-5 md:h-5 rounded-full bg-green-500 shadow-sm border border-green-600"></div>
                                    ))}
                                </div>
                            ))}
                         </div>
