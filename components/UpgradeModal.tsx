      ...
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div className="wood-panel p-4 md:p-8 w-full max-w-sm md:max-w-xl landscape:max-w-2xl landscape:flex landscape:flex-row landscape:gap-6 relative">
           ...
           <div className="flex flex-col items-center mb-6 landscape:mb-0 landscape:flex-1 justify-center">
              ...
              <h2 className="text-xl md:text-2xl font-black text-yellow-100 mb-2 drop-shadow-md">{config.name} Lv.{plant.level}</h2>
              ...
              {currentDamage > 0 && (
                <div className="bg-orange-900/50 px-3 py-1 rounded-full border border-orange-500 text-[10px] md:text-xs text-orange-200 mt-2 font-bold">
                  ⚔️ DAMAGE: {currentDamage}
                </div>
              )}
              {jalBoost && (
                <div className="bg-red-900/50 px-3 py-1 rounded-full border border-red-500 text-[10px] md:text-xs text-red-200 mt-2 animate-pulse font-bold">
                  🔥 BOOST: x{jalBoost}
                </div>
              )}
           </div>
           ...
           <div className="grid grid-cols-2 landscape:grid-cols-1 gap-4 w-full landscape:flex-1 justify-center">
                <div className="flex gap-4 col-span-2 landscape:col-span-1">
                    <button onClick={() => startChallenge('UPGRADE_CHALLENGE')} className="flex-1 glossy-btn bg-yellow-400 text-white py-4 font-black border-none text-sm md:text-base">UPGRADE</button>
                    <button onClick={() => startChallenge('HEAL_CHALLENGE')} className="flex-1 glossy-red bg-pink-500 text-white py-4 font-black border-none text-sm md:text-base">HEAL</button>
                </div>
                <button onClick={() => onRemove(plant.id)} className="col-span-2 landscape:col-span-1 bg-stone-600 hover:bg-stone-500 text-white py-4 rounded-lg font-black border-b-4 border-stone-800 active:border-b-0 active:translate-y-1 text-sm md:text-base">
                  REMOVE (+{Math.floor(config.cost * 0.5)})
                </button>
                <p className="text-[10px] md:text-xs text-stone-300 text-center mt-2 opacity-80 font-bold">Get a streak to grow stronger!</p>
           </div>
        </div>
      </div>
    );
       ...
       <div className={`wood-panel p-4 sm:p-6 md:p-8 w-full max-w-md landscape:max-w-2xl md:max-w-3xl ${shake ? 'animate-pulse' : ''}`}>
          <div className="flex justify-between items-center mb-4">
              <div className="flex gap-4 text-sm md:text-lg">
                  <span className={mode === 'HEAL_CHALLENGE' ? "text-pink-400 font-black" : "text-yellow-400 font-black"}>{mode === 'HEAL_CHALLENGE' ? "❤️ HEAL" : "⭐ UPGRADE"}</span>
                  <span className={`text-white font-black ${comboFlash ? 'scale-125 text-yellow-300' : ''} transition-transform`}>COMBO: {streak}</span>
              </div>
              <div className="flex gap-2">
                <button 
                  onClick={onOpenStudy}
                  className={`bg-purple-600 text-white px-3 py-1 rounded-full border-2 border-purple-800 shadow-md text-xs md:text-sm font-black flex items-center gap-1 hover:bg-purple-500 active:scale-95 ${isStudyOpen ? 'ring-4 ring-yellow-400' : ''}`}
                >
                  <span>📖</span> {isStudyOpen ? "STUDYING..." : "STUDY"}
                </button>
                {!isStudyOpen && <button onClick={onClose} className="text-white font-black text-xl hover:text-red-400">X</button>}
              </div>
          </div>
          ...
          <div className="flex flex-col landscape:flex-row landscape:gap-4 md:gap-8 relative">
              {isStudyOpen && (
                  <div className="absolute inset-0 bg-black/40 backdrop-blur-[2px] z-20 flex items-center justify-center rounded-xl border-4 border-dashed border-purple-500/50">
                      <div className="text-white font-black text-2xl animate-pulse uppercase">Game Paused</div>
                  </div>
              )}

              <div className="landscape:flex-1">
                <div className="text-center text-[10px] md:text-sm mb-2 text-stone-300 font-bold">
                    {mode === 'HEAL_CHALLENGE' ? "Solve 3 in a row!" : "Solve 2+ for Level Up!"}
                </div>
              ...
              <div className="landscape:flex-1">
                <div className="grid grid-cols-3 gap-2 md:gap-4">
                    ...
                    <button onClick={handleBackspace} className="glossy-red bg-red-500 text-white text-sm md:text-xl py-3 rounded-lg font-black">DEL</button>
                    <button onClick={handleSubmit} className="bg-green-500 text-white text-sm md:text-xl py-3 rounded-lg border-b-4 border-green-700 active:border-0 active:translate-y-1 font-black">GO</button>
                </div>
              </div>
          </div>
       </div>
